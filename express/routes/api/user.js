...
router.get('/', JwtAuth, verifyRole(ROLE.Admin_it, ROLE.Admin_pr, ROLE.Super_admin), async (req, res) => {
    ...
    try {
        ...
        // get user when pool
        let { status_pool: status_p, data: users, msg: msg } = await connMSQL.connection_pool(validator.foundId(viewTable))
...
router.post('/', JwtAuth, verifyRole(ROLE.Super_admin), async (req, res) => {
    ...
    try {
        input = [
            ...
            { prop: "user_position", value: validator.validateStrNotNull(await req.body.user_position, 80, table, 'user_position'), type: 'str' },
